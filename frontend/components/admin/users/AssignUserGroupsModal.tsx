...
export function AssignUserGroupsModal({ user, onClose }: { user: UserResponse | null; onClose: () => void }) {
  ...
  return (
    <Modal isOpen={Boolean(user)} onClose={onClose} title={`${user?.displayName ?? ""} — Gruplar`} size="sm">
      {isLoading && <Spinner className="mx-auto text-primary-500" />}
      <div className="flex max-h-64 flex-col gap-1 overflow-y-auto">
        {groups?.map((group) => (
          <label key={group.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm text-zinc-700 hover:bg-zinc-50">
            <input
              type="checkbox"
        ...
        {groups && groups.length === 0 && <p className="px-2 py-4 text-center text-sm text-zinc-400">Henüz grup yok</p>}
      </div>

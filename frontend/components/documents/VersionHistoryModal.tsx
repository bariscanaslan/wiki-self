    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Versiyon Geçmişi" size="lg">
      <div className="flex h-[28rem] gap-4">
        <div className="w-48 shrink-0 overflow-y-auto border-r border-zinc-100 pr-3">
          {isLoading && <Spinner className="mx-auto mt-6 text-primary-500" />}
          {versions?.map((version) => (
            <button
              ...
              className={cn(
                ...
                selectedVersionId === version.id ? "bg-primary-50 text-primary-700" : "text-zinc-600 hover:bg-zinc-100",
              )}
            >
              <span className="font-semibold">
                v{version.versionNumber} {version.versionNumber === currentVersionNumber && "(güncel)"}
              </span>
              <span className="text-zinc-400">{version.authorDisplayName}</span>
              <span className="text-zinc-400">{format(new Date(version.createdAt), "d MMM yyyy HH:mm", { locale: tr })}</span>
            </button>
          ))}
        </div>

        <div className="flex-1 overflow-y-auto pr-1">
          {!selectedVersionId && <p className="mt-10 text-center text-sm text-zinc-400">İncelemek için bir versiyon seçin</p>}
          {selectedVersionId && isVersionLoading && <Spinner className="mx-auto mt-10 text-primary-500" />}
          {versionDetail && (
            <div>

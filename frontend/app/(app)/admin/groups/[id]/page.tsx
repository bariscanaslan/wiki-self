...
export default function GroupDetailPage() {
  ...
  return (
    <div>
      <Link href="/admin/groups" className="mb-4 inline-flex items-center gap-1 text-sm text-zinc-500 hover:text-primary-600">
        <ArrowLeft size={14} /> Gruplara dön
      </Link>
      <h2 className="mb-1 text-xl font-bold text-zinc-900">{group.name}</h2>
      {group.description && <p className="mb-6 text-sm text-zinc-500">{group.description}</p>}

      <div className="mb-4 flex gap-2">
        <select
          value={pendingUserId}
          onChange={(event) => setPendingUserId(event.target.value)}
          className="flex-1 rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:border-primary-400 focus:outline-none"
        >
            ...
            {group.members.map((member) => (
              <tr key={member.id}>
                <td className="px-4 py-3">
                  <div className="font-medium text-zinc-900">{member.displayName}</div>
